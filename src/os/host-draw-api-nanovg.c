...
#define REBCNT_NVG_COLOR(c) \
		nvgRGBA(reinterpret_cast<unsigned char*>(&(c))[C_R], \
			   reinterpret_cast<unsigned char*>(&(c))[C_G], \
			   reinterpret_cast<unsigned char*>(&(c))[C_B], \
			   reinterpret_cast<unsigned char*>(&(c))[C_A])
...
#define PAINT_LAYER_FULL(ctx, layer, paint_mode) 	\
	...
	extern "C" void rebdrw_gob_color(REBGOB *gob, REBDRW_CTX *ctx, REBXYI abs_oft, REBXYI clip_top, REBXYI clip_bottom)
	{
	...
	extern "C" void rebdrw_gob_image(REBGOB *gob, REBDRW_CTX *ctx, REBXYI abs_oft, REBXYI clip_top, REBXYI clip_bottom)
	{
	...
	extern "C" void rebdrw_gob_draw(REBGOB *gob, REBDRW_CTX *ctx, REBXYI abs_oft, REBXYI clip_top, REBXYI clip_bottom)
	{
		REBINT result;
	   	REBCEC cec_ctx;
	...
	extern "C" REBDRW_CTX* rebdrw_create_context(REBINT w, REBINT h)
	{
	...
	extern "C" void rebdrw_resize_context(REBDRW_CTX *ctx, REBINT w, REBINT h)
	{
		ctx->ww = w;
		ctx->wh = h;
		delete_layers(ctx);
	...
	extern "C" void rebdrw_destroy_context(REBDRW_CTX *ctx)
	{
		if (ctx == NULL) return;
	...
	extern "C" void rebdrw_begin_frame(REBDRW_CTX *ctx)
	{
	...
	extern "C" void rebdrw_end_frame(REBDRW_CTX *ctx)
	{
		nvgEndLayer(ctx->nvg, ctx->win_layer);
		//printf("End frame: %d\n", __LINE__);
		nvgEndFrame(ctx->nvg);
	...
	extern "C" void rebdrw_blit_frame(REBDRW_CTX *ctx)
	{
		nvgBeginFrame(ctx->nvg, ctx->ww, ctx->wh, ctx->pixel_ratio);
		glClearColor(0, 0, 0, 0);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
