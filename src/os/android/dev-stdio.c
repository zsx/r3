...
************************************************************************
**
**  Title: Device: Standard I/O for Android
...
***********************************************************************/
{
	...
	if (Std_Out >= 0) {
		...
		s = (REBCHR*)Make_Mem(req->length+1);
		COPY_STR(s, req->data, req->length);
		jstr = (*jni_env)->NewStringUTF(jni_env, s);
		Free_Mem(s, req->length+1);
		
		(*jni_env)->CallVoidMethod(jni_env, jni_obj, jni_putOutput, jstr);
...
//			total = javaInput(req->data, len);

			REBCHR *rstr;
			jstring result;
			
			if (jni_env == NULL) return 0;
			
			result = (*jni_env)->CallObjectMethod(jni_env, jni_obj, jni_getInput);
			rstr = (REBCHR*)(*jni_env)->GetStringUTFChars(jni_env, result, NULL);
			total = (*jni_env)->GetStringUTFLength(jni_env, result);
			COPY_STR(req->data, rstr, total);
