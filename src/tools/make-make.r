...
{# REBOL Makefile -- Generated by make-make.r (!!! EDITS MAY BE LOST !!!)
# This automatically produced file was created !date

# This makefile is intentionally kept simple to make builds possible on
# a wide range of target platforms.  Although it is a generated file, a
# version is committed into the repository so that you can cause the first
# generation just by typing:
#
#	make make
#
# You will require a Rebol 3 interpreter to be in the directory named
# r3-make (or r3-make.exe on Windows).  The bootstrap script is intended
# to stay as stable as possible, so you *should* even be able to use an
# old executable from the pre-open-source Rebol3 downloads on rebol.com:
#
#	http://www.rebol.com/r3/downloads.html
#
# (If you notice bootstrap with an old interpreter has has broken on your
# system, please report it!  Few are testing with executables that old ATM.)
#
# For a more recent download, try getting your r3-make from:
#
#	http://rebolsource.net/
#
# By default, when you run 'make make' the make-make.r script will assume
# that you want to use the same OS_ID as that executable has.  That's usually
# probably what you want.  But to generate this file for a different platform,
# (e.g. cross-compilation) or if it just plain guesses wrong, check the
# %src/tools/systems.r file, and provide an OS_ID from the table.  For
# example, Linux with clib 2.5:
#
#    make make OS_ID=0.4.3
#
# Note that these numbers are what appears at the tail of a full Rebol version
# number.  So for instance you might find the numbers above in a tuple like
# `2.101.0.4.3`, where the first numbers are referring to the version of the
# actual Rebol codebase itself.
#
