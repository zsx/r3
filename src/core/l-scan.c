...
REBVAL *Scan_To_Stack(SCAN_STATE *ss) {
    ...
    while (
        ...
        case TOKEN_SET:
            len--;
            if (ss->mode_char == '/' && ss->token == TOKEN_SET) {
        ...
        case TOKEN_TIME:
            if (
                bp[len - 1] == ':'
                && ss->mode_char == '/' // could be path/10: set
            ){
                DS_PUSH_TRASH;
        ...
        case TOKEN_DATE:
            while (*ep == '/' && ss->mode_char != '/') {  // Is it date/time?
                ep++;
