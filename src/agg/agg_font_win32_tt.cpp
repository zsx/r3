...
#define GGO_UNHINTED 0x0100
            ...
            if(total_size < 0)
            {
				...
				if(total_size < 0) {
					//Fixed size bitmap font - use TextOut to get the glyph bitmap
					SIZE area;
					GetTextExtentPoint32(m_dc, (wchar_t*)&glyph_code, 1, &area);
					TEXTMETRIC tm;
					...
					area.cx+=tm.tmOverhang;

					gm.gmBlackBoxX = area.cx;
					gm.gmBlackBoxY = area.cy;
					gm.gmptGlyphOrigin.x=0;
					gm.gmptGlyphOrigin.y=area.cy - tm.tmDescent;

					HBITMAP hbm = CreateBitmap(area.cx, area.cy, 1, 1, 0);
					HDC hbdc = CreateCompatibleDC(m_dc);
					SelectObject(hbdc, hbm);
					SelectObject(hbdc, m_cur_font);
					SetBkColor(hbdc,RGB(0,0,0));
					SetTextColor(hbdc,RGB(255,255,255));

					TextOut(hbdc,0,0,(wchar_t*)&glyph_code, 1);
					DeleteDC(hbdc);

					GetBitmapBits(hbm, area.cx * area.cy, m_gbuf);
					DeleteObject(hbm);
