...
**  Summary: REBOL EFFECTS dialect rendering functions
...
//Reb_Print("%dx%d", m_rbas_dst->xmin(),m_rbas_dst->ymin());
	...
	void agg_effects::get_offset(REBXYI* p){
		p->x = m_src_x;
		p->y = m_src_y;
	}

	void agg_effects::get_size(REBXYI* p){
		p->x = m_src_w;
		p->y = m_src_h;
	}
	...
	***********************************************************************/
	...
	void agg_effects::fx_crop(REBXYI* p1, REBXYI* p2, unsigned char *src, int src_w, int src_h){
	...
	void agg_effects::fx_extend(REBXYI* p1, REBXYI* p2, unsigned char *src, int src_w, int src_h){
		agg_graphics::pixfmt pixf_src;
		rendering_buffer rbuf_src;
		REBINT y1,x1;
		if (src) {
	...
	void agg_effects::fx_flip(REBXYI* p, unsigned char *src, int src_w, int src_h){
		if (src) {
			int w = MIN(src_w - m_clip.x1,m_clip_w);
			if (w > 0) {
				int h = MIN(src_h - m_clip.y1,m_clip_h);
	...
	void agg_effects::fx_gradcol(REBXYI* p, REBYTE* color1, REBYTE* color2, unsigned char *src, int src_w, int src_h){
		if (src) {
	...
	void agg_effects::fx_gradient(REBXYI* p, REBYTE* color1, REBYTE* color2, unsigned char *src, int src_w, int src_h, unsigned char *dst, int dst_w, int dst_h){
		if (src){
			rendering_buffer rbuf_src(src,src_w,src_h,src_w * 4);
			agg_graphics::pixfmt pixf_src(rbuf_src);
			m_rbas_dst->blend_from(pixf_src,0,0,0, 255 - m_alpha);
	...
	void agg_effects::fx_gradmul(REBXYI* p, REBYTE* color1, REBYTE* color2, unsigned char *src, int src_w, int src_h){
		if (src) {
			unsigned char* srcg = new unsigned char[src_w * src_h * 4];
			fx_gradient(p,color1,color2, 0 ,0 ,0 , srcg, src_w, src_h);
...
//			m_rbas_dst->for_each_pixel(fx_luma(brightness), *m_rbuf_dst);
	...
	void agg_effects::fx_reflect(REBXYI* p, unsigned char *src, int src_w, int src_h){
	...
	void agg_effects::fx_shadow(unsigned char *src, int src_w, int src_h, REBXYI* p1,REBXYI* p2, REBYTE* color, REBDEC blur, REBINT only){
