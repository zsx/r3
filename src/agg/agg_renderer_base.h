...
namespace agg
{
    ...
    public:
        ...
        {
#ifdef AGG_OPENGL
            glClearColor( c.r *color_factor, c.g * color_factor ,c.b * color_factor, 0.0f);
...
*/
                ...
                rc.y2++;
                glBegin( GL_QUADS );
                glColor3f( c.r * color_factor, c.g * color_factor ,c.b * color_factor);
...
//                glEnd();
                if (c.a){
                    int32u alpha = (int32u(c.a) * (int32u(cover) + 1)) >> 8;
        ...
        //--------------------------------------------------------------------
        void copy_from2(const rendering_buffer& src, 
                       const rect* rect_src_ptr = 0, 
                       int dx = 0, 
                       int dy = 0)
        {
            ...
            rect rc = clip_rect_area(rdst, rsrc, src.width(), src.height());
//Reb_Print("RC: %d %d %d %d", rc.x1, rc.y1, rc.x2, rc.y2);
            if(rc.x2 > 0)
            {
                int incy = 1;
                if(rdst.y1 > rsrc.y1)
        ...
        template<class Function> void for_each_pixel(Function f, const rendering_buffer& src)
        {
			int y;
			unsigned l = unsigned(m_clip_box.x2 - m_clip_box.x1 + 1);

            for(y = m_clip_box.y1; y <= m_clip_box.y2; y++)
            {
				unsigned len = l;
				value_type* p = (value_type*)src.row(y) + (m_clip_box.x1 << 2);
				do
				{
					f(p);
					p += 4;
